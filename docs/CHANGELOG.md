...
首个正式版本，累积 v0.1.0 ~ v0.5.2 全部功能与修复。
...
- **detail 模式推送图片只发送 URL**：新增 `_send_image` 方法通过 `Image` 组件发送真实 QQ 图片
  - `_format_post_detail_text` 只输出文本，图片由 `_send_image` 单独发送
...
- **新消息推送展示方式可选**：新增 `push_mode` 配置项
  - `list`（原有形式）：标题列表 + 🆕 标识
  - `detail`：先发标题列表，再逐条展开完整详情（含有序图文和图片链接）
- 关键词触发的列表和序号交互详情功能不受影响

### 调整

- `_conf_schema.json` 新增 `push_mode` 配置项（options: list/detail）
- `push_handler` 新增 `_format_post_detail()` 方法
...
- **`/刷新百分之一消息` 命令**：管理员可立即触发一次爬取刷新缓存
  - 仅管理员可用（受 `admin_qq` 控制）
  - 先回复"正在刷新"，完成后回复当前缓存总数
- 抽象 `_check_admin()` 方法，减少权限检查重复代码
...
- **插件加载/重载后自动执行首次爬取**：不再等待用户首次发送关键词
  - 通过 `loop.call_soon` 在事件循环就绪后延迟调度 `_do_initial_crawl`
  - 同时保留 `on_astrbot_loaded` 和 `_ensure_crawled` 作为兜底
...
- **黑名单模式不推送**：改为记录所有交互过的群/用户作为推送目标
  - `AccessControl` 维护 `_seen_groups` / `_seen_privates`
  - 每次消息事件自动记录群号和用户 QQ
  - `get_push_targets()` 返回所有见过的会话（已按黑白名单过滤）
